<x-app-layout title="購入馬券作成（{{ $race->name }}）">
    ...
    <style>
        #wideFormation .combo-scroll {
            scrollbar-width: thin;
        }

        #wideFormation .combo-scroll::-webkit-scrollbar {
            width: 6px;
            height: 6px;
        }

        #wideFormation .combo-scroll::-webkit-scrollbar-thumb {
            background: #cbd5e1;
            border-radius: 9999px;
        }

        #wideFormation .combo-scroll::-webkit-scrollbar-track {
            background: transparent;
        }
    </style>

    <div id="wideFormation" class="space-y-4">
        ...
        <form method="POST" action="{{ route('bet.cart.add', $race) }}" class="space-y-4">
            ...
            <div class="bg-white rounded-xl shadow-sm ring-1 ring-gray-200 p-4 space-y-4">
                <div class="flex items-center justify-between gap-3">
                    <h2 class="text-base font-semibold">ワイド フォーメーション</h2>
                </div>

                <div class="text-sm text-gray-600 leading-relaxed">
                    1列目×2列目から組み合わせを作ります（同一馬は除外）。<br>
                    ...
                    <div class="lg:col-span-2">
                        <div class="flex items-end justify-between mb-2 gap-2">
                            <div>
                                ...
                                <div class="text-xs text-gray-500">各列1頭以上（同一馬は除外）</div>
                            </div>
                            ...
                            <table class="min-w-full text-sm">
                                <thead class="bg-gray-50">
                                    <tr>
                                        ...
                                        <th class="px-3 py-2 text-center font-medium text-gray-600 w-28">1列目</th>
                                        <th class="px-3 py-2 text-center font-medium text-gray-600 w-28">2列目</th>
                                    </tr>
                                </thead>
                                ...
                                <tbody id="horseGrid" class="divide-y divide-gray-100 bg-white">
                                    @foreach ($horsesSorted as $no)
                                        ...
                                        <tr data-row="{{ $noStr }}" class="hover:bg-gray-50">
                                            ...
                                            {{-- 1列目 --}}
                                            <td class="px-3 py-2 text-center cursor-pointer" data-col="first"
                                                data-no="{{ $noStr }}">
                                                <label
                                            ...
                                            {{-- 2列目 --}}
                                            <td class="px-3 py-2 text-center cursor-pointer" data-col="second"
                                                data-no="{{ $noStr }}">
                                                <label
                                                    class="inline-flex items-center justify-center w-full py-2 rounded hover:bg-gray-100 cursor-pointer
                                                              focus-within:ring-2 focus-within:ring-indigo-500">
                        ...
                        <div class="sticky top-4 space-y-4">
                            ...
                            <div class="rounded-xl ring-1 ring-gray-200 bg-white overflow-hidden">
                                <div class="bg-indigo-600 text-white px-4 py-3">
                                    <div class="text-sm font-semibold">ワイド フォーメーション</div>
                                </div>
    ...
    <script>
        (() => {
            ...
            // 1列×2列 -> unordered pair（昇順固定）を重複排除（同一馬は除外）
            function buildKeys(A, B) {
                const set = new Set();

                for (const a of A) {
                    for (const b of B) {
                        if (a === b) continue; // ✅ 同一馬は除外
