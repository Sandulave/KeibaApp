<x-app-layout title="購入馬券作成（{{ $race->name }}）">
    ...
    <style>
        #wakurenFormation .combo-scroll {
            scrollbar-width: thin;
        }

        #wakurenFormation .combo-scroll::-webkit-scrollbar {
            width: 6px;
            height: 6px;
        }

        #wakurenFormation .combo-scroll::-webkit-scrollbar-thumb {
            background: #cbd5e1;
            border-radius: 9999px;
        }

        #wakurenFormation .combo-scroll::-webkit-scrollbar-track {
            background: transparent;
        }
    </style>

    <div id="wakurenFormation" class="space-y-4">
        ...
        <form method="POST" action="{{ route('bet.cart.add', $race) }}" class="space-y-4">
            ...
            <div class="bg-white rounded-xl shadow-sm ring-1 ring-gray-200 p-4 space-y-4">
                <div class="flex items-center justify-between gap-3">
                    <h2 class="text-base font-semibold">枠連 フォーメーション</h2>
                </div>
                                ...
                                <tbody id="frameGrid" class="divide-y divide-gray-100 bg-white">
                                    @foreach ($availableFrames as $frame)
                                        @php
                                            ...
                                            $horsesInFrame = $frameToHorses[$frame] ?? [];
                                        @endphp
                                        ...
                                        <tr data-row="{{ $frameStr }}" class="hover:bg-gray-50">
                                            ...
                                            <td class="px-3 py-2 cursor-pointer" data-col="first"
                                                ...
                                                <label
                                                    ...
                                                    <input type="checkbox" name="first[]" value="{{ $frameStr }}"
                                                        class="h-4 w-4 rounded border-gray-300 text-indigo-600"
                                                        aria-label="1列目：枠{{ $frameStr }}"
                                                        @checked(in_array($frameStr, $oldFirst, true))>
                                            ...
                                            <td class="px-3 py-2 cursor-pointer" data-col="second"
                                                ...
                                                <label
                                                    class="inline-flex items-center gap-2 cursor-pointer select-none px-2 py-1 rounded hover:bg-gray-100
                                                              focus-within:ring-2 focus-within:ring-indigo-500">
                                                    <input type="checkbox" name="second[]" value="{{ $frameStr }}"
                                                        class="h-4 w-4 rounded border-gray-300 text-indigo-600"
                                                        aria-label="2列目：枠{{ $frameStr }}"
                                                        @checked(in_array($frameStr, $oldSecond, true))>
                        ...
                        <div class="sticky top-4 space-y-4">
                            ...
                            <div class="rounded-xl ring-1 ring-gray-200 bg-white overflow-hidden">
                                <div class="bg-indigo-600 text-white px-4 py-3">
                                    <div class="text-sm font-semibold">枠連 フォーメーション</div>
                                </div>
        ...
        (() => {
            ...
            function buildKeys(A, B) {
                ...
                return Array.from(set).sort((k1, k2) => {
                    const a1 = k1.split('-').map(n => parseInt(n, 10) || 0);
                    const a2 = k2.split('-').map(n => parseInt(n, 10) || 0);
            ...
            function updateRowHighlight(firstSet, secondSet) {
                grid.querySelectorAll('tr[data-row]').forEach(tr => {
                    ...
                    const on = firstSet.has(v) || secondSet.has(v);
            ...
            grid.addEventListener('click', (e) => {
                ...
                const cb = td.querySelector(`${sel}[value="${frame}"]`);
                if (!cb) return;
            ...
            grid.addEventListener('change', (e) => {
                if (e.target.matches(firstSelector) || e.target.matches(secondSelector)) updateAll();
            });

            function setAll(sel, checked) {
                grid.querySelectorAll(sel).forEach(cb => cb.checked = checked);
                updateAll();
            }
