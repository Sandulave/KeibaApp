<x-app-layout title="購入馬券作成（{{ $race->name }}）">
    ...
    <style>
        #sanrenpukuBox .combo-scroll {
            scrollbar-width: thin;
        }

        #sanrenpukuBox .combo-scroll::-webkit-scrollbar {
            width: 6px;
            height: 6px;
        }

        #sanrenpukuBox .combo-scroll::-webkit-scrollbar-thumb {
            background: #cbd5e1;
            border-radius: 9999px;
        }

        #sanrenpukuBox .combo-scroll::-webkit-scrollbar-track {
            background: transparent;
        }
    </style>

    <div id="sanrenpukuBox" class="space-y-4">
        ...
        <form method="POST" action="{{ route('bet.cart.add', $race) }}" class="space-y-4">
            ...
            <div class="bg-white rounded-xl shadow-sm ring-1 ring-gray-200 p-4 space-y-4">
                <div class="flex items-center justify-between gap-3">
                    <h2 class="text-base font-semibold">三連複 ボックス</h2>
                </div>

                <div class="text-sm text-gray-600 leading-relaxed">
                    馬番を3頭以上選ぶと、組み合わせ（C(n,3)）で自動生成します。<br>
                                        ...
                                        <tr data-row="{{ $noStr }}" class="hover:bg-gray-50">
                                            ...
                                            <td class="px-3 py-2 cursor-pointer" data-col="horse"
                                                ...
                                                <label
                                                    ...
                                                    <input type="checkbox" name="horses[]" value="{{ $noStr }}"
                        ...
                        <div class="sticky top-4 space-y-4">
                            ...
                            <div class="rounded-xl ring-1 ring-gray-200 bg-white overflow-hidden">
                                <div class="bg-indigo-600 text-white px-4 py-3">
                                    <div class="text-sm font-semibold">三連複 ボックス</div>
                                </div>
    ...
    <script>
        (() => {
            ...
            function buildCombos(horses, limit = MAX_COMBO_PREVIEW) {
                const combos = [];
                const n = horses.length;
                const total = comb(n, 3);
                ...
                for (let i = 0; i < n; i++) {
                    for (let j = i + 1; j < n; j++) {
                        for (let k = j + 1; k < n; k++) {
                            if (combos.length < limit) combos.push([horses[i], horses[j], horses[k]]);
                        }
                    }
                }
            ...
            function renderCombos(horses) {
                const {
                    combos,
                    total
                } = buildCombos(horses, MAX_COMBO_PREVIEW);
            ...
            horseGrid.addEventListener('change', (e) => {
                if (e.target.matches(horseSelector)) {
                    updateAll();
                }
            });
