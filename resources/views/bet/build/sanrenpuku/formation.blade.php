<x-app-layout title="購入馬券作成（{{ $race->name }}）">
    ...
    <style>
        #sanrenpukuFormation .combo-scroll {
            scrollbar-width: thin;
        }

        #sanrenpukuFormation .combo-scroll::-webkit-scrollbar {
            width: 6px;
            height: 6px;
        }

        #sanrenpukuFormation .combo-scroll::-webkit-scrollbar-thumb {
            background: #cbd5e1;
            border-radius: 9999px;
        }

        #sanrenpukuFormation .combo-scroll::-webkit-scrollbar-track {
            background: transparent;
        }
    </style>

    <div id="sanrenpukuFormation" class="space-y-4">
        ...
        <form method="POST" action="{{ route('bet.cart.add', $race) }}" class="space-y-4">
            ...
            <input type="hidden" name="betType" value="{{ $betType ?? 'sanrenpuku' }}">
            ...
            <div class="bg-white rounded-xl shadow-sm ring-1 ring-gray-200 p-4 space-y-4">
                <div class="flex items-center justify-between gap-3">
                    <h2 class="text-base font-semibold">三連複 フォーメーション</h2>
                </div>

                <div class="text-sm text-gray-600 leading-relaxed">
                    1列目×2列目×3列目 から3頭を作ります（同一馬の重複は除外）。<br>
                    三連複は順不同なので、表示は <strong>昇順（1-2-3）</strong> に固定します。
                </div>
                    ...
                    <div class="lg:col-span-2">
                        <div class="flex items-end justify-between mb-2 gap-2">
                            <div>
                                ...
                                <div class="text-xs text-gray-500">各列1頭以上</div>
                            </div>
                            ...
                            <div class="flex gap-2 flex-wrap justify-end">
                                ...
                                <button type="button" id="thirdAll"
                                    class="px-3 py-2 rounded-lg bg-white border text-sm hover:bg-gray-100">
                                    3列 全通り
                                </button>
                                <button type="button" id="thirdClear"
                                    class="px-3 py-2 rounded-lg bg-white border text-sm hover:bg-gray-100">
                                    3列 クリア
                                </button>
                            </div>
                            ...
                            <table class="min-w-full text-sm">
                                <thead class="bg-gray-50">
                                    <tr>
                                        ...
                                        <th class="px-3 py-2 text-left font-medium text-gray-600 w-40">1列目</th>
                                        <th class="px-3 py-2 text-left font-medium text-gray-600 w-40">2列目</th>
                                        <th class="px-3 py-2 text-left font-medium text-gray-600 w-40">3列目</th>
                                    </tr>
                                </thead>
                                            ...
                                            <td class="px-3 py-2 cursor-pointer" data-col="first"
                                                ...
                                                <label
                                                    ...
                                                    <input type="checkbox" name="first[]" value="{{ $noStr }}"
                                                        ...
                                                        aria-label="1列目：馬番{{ $noStr }}"
                                                        @checked(in_array($noStr, $oldFirst, true))>
                                                    <span class="text-gray-700">1列</span>
                                                </label>
                                            </td>
                                            ...
                                            <td class="px-3 py-2 cursor-pointer" data-col="second"
                                                ...
                                                <label
                                                    ...
                                                    <input type="checkbox" name="second[]" value="{{ $noStr }}"
                                                        class="h-4 w-4 rounded border-gray-300 text-indigo-600"
                                                        aria-label="2列目：馬番{{ $noStr }}"
                                                        @checked(in_array($noStr, $oldSecond, true))>
                                                    <span class="text-gray-700">2列</span>
                                                </label>
                                            </td>

                                            {{-- 3列目 --}}
                                            <td class="px-3 py-2 cursor-pointer" data-col="third"
                                                data-no="{{ $noStr }}">
                                                <label
                                                    class="inline-flex items-center gap-2 cursor-pointer select-none px-2 py-1 rounded hover:bg-gray-100
                                                              focus-within:ring-2 focus-within:ring-indigo-500">
                                                    <input type="checkbox" name="third[]" value="{{ $noStr }}"
                                                        class="h-4 w-4 rounded border-gray-300 text-indigo-600"
                                                        aria-label="3列目：馬番{{ $noStr }}"
                                                        @checked(in_array($noStr, $oldThird, true))>
                                                    <span class="text-gray-700">3列</span>
                                                </label>
                                            </td>
                                        </tr>
                                    @endforeach
                        ...
                        <div class="sticky top-4 space-y-4">
                            ...
                            <div class="rounded-xl ring-1 ring-gray-200 bg-white overflow-hidden">
                                <div class="bg-indigo-600 text-white px-4 py-3">
                                    <div class="text-sm font-semibold">三連複 フォーメーション</div>
                                </div>
                                ...
                                <div class="p-4 space-y-3">
                                    ...
                                    <div class="space-y-2">
                                        <div>
                                            <div class="text-xs text-gray-500 mb-1">1列目</div>
                                            <div id="firstInline" class="text-sm text-gray-800">未選択</div>
                                        </div>
                                        <div>
                                            <div class="text-xs text-gray-500 mb-1">2列目</div>
                                            <div id="secondInline" class="text-sm text-gray-800">未選択</div>
                                        </div>
                                        <div>
                                            <div class="text-xs text-gray-500 mb-1">3列目</div>
                                            <div id="thirdInline" class="text-sm text-gray-800">未選択</div>
                                        </div>
                                    </div>
    ...
    <script>
        (() => {
            ...
            const thirdInlineEl = document.getElementById('thirdInline');
            ...
            const thirdAllBtn = document.getElementById('thirdAll');
            const thirdClearBtn = document.getElementById('thirdClear');

            const firstSelector = 'input[name="first[]"]';
            const secondSelector = 'input[name="second[]"]';
            const thirdSelector = 'input[name="third[]"]';
            ...
            // Builderのロジックに合わせて：
            // first×second×third で (a=b) (c=a/b) を除外し、unordered3で重複排除
            function buildKeys(A, B, C) {
                const set = new Set();
                ...
                for (const a of A) {
                    for (const b of B) {
                        ...
                        for (const c of C) {
                            ...
                            const arr = [a, b, c].slice().sort((x, y) => (parseInt(x, 10) || 0) - (parseInt(y, 10) ||
                                0));
            ...
            function renderCombos(keys) {
                ...
                for (const k of shown) {
                    ...
                    div.innerHTML = `
                        ...
                        <span class="text-gray-400">-</span>
                        <span class="px-2 py-1 bg-white rounded border text-xs font-semibold">${c}</span>
                    `;
                    comboWrap.appendChild(div);
                }
            }

            function updateRowHighlight(firstSet, secondSet, thirdSet) {
                horseGrid.querySelectorAll('tr[data-row]').forEach(tr => {
                    const no = tr.getAttribute('data-row');
                    const on = firstSet.has(no) || secondSet.has(no) || thirdSet.has(no);
            ...
            horseGrid.addEventListener('click', (e) => {
                ...
                if (col === 'third') sel = thirdSelector;
                if (!sel) return;
            ...
            horseGrid.addEventListener('change', (e) => {
                if (e.target.matches(firstSelector) || e.target.matches(secondSelector) || e.target.matches(
                        thirdSelector)) {
                    updateAll();
                }
            });
            ...
            thirdAllBtn.addEventListener('click', () => setAll(thirdSelector, true));
            thirdClearBtn.addEventListener('click', () => setAll(thirdSelector, false));
