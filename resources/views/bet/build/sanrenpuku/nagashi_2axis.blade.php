<x-app-layout title="購入馬券作成（{{ $race->name }}）">
    ...
    <style>
        #sanrenpukuNagashi2Axis .combo-scroll {
            scrollbar-width: thin;
        }

        #sanrenpukuNagashi2Axis .combo-scroll::-webkit-scrollbar {
            width: 6px;
            height: 6px;
        }

        #sanrenpukuNagashi2Axis .combo-scroll::-webkit-scrollbar-thumb {
            background: #cbd5e1;
            border-radius: 9999px;
        }

        #sanrenpukuNagashi2Axis .combo-scroll::-webkit-scrollbar-track {
            background: transparent;
        }
    </style>

    <div id="sanrenpukuNagashi2Axis" class="space-y-4">
        ...
        <form method="POST" action="{{ route('bet.cart.add', $race) }}" class="space-y-4">
            ...
            <div class="bg-white rounded-xl shadow-sm ring-1 ring-gray-200 p-4 space-y-4">
                <div class="flex items-center justify-between gap-3">
                    <h2 class="text-base font-semibold">三連複 2頭軸流し</h2>
                </div>
                        ...
                        <div class="flex items-end justify-between mb-2 gap-2">
                            <div>
                                ...
                                <div class="text-xs text-gray-500">
                                    軸1・軸2は各1頭（同じ馬は選べません）／相手は1頭以上（軸と同じ馬は相手にできません）
                                </div>
                            </div>
                            ...
                            <table class="min-w-full text-sm">
                                <thead class="bg-gray-50">
                                    <tr>
                                        ...
                                        <th class="px-3 py-2 text-left font-medium text-gray-600 w-40">軸1</th>
                                        <th class="px-3 py-2 text-left font-medium text-gray-600 w-40">軸2</th>
                        ...
                        <div class="sticky top-4 space-y-4">
                            ...
                            <div class="rounded-xl ring-1 ring-gray-200 bg-white overflow-hidden">
                                <div class="bg-indigo-600 text-white px-4 py-3">
                                    <div class="text-sm font-semibold">三連複 2頭軸流し</div>
                                </div>
                                ...
                                <div class="p-4 space-y-3">
                                    ...
                                    <div class="space-y-2">
                                        <div>
                                            ...
                                            <div id="axis1Inline" class="text-sm text-gray-800">未選択</div>
                                        </div>
                                        <div>
                                            <div class="text-xs text-gray-500 mb-1">軸2</div>
                                            <div id="axis2Inline" class="text-sm text-gray-800">未選択</div>
                                        </div>
                                        <div>
    ...
    <script>
        (() => {
            ...
            const axis1InlineEl = document.getElementById('axis1Inline');
            const axis2InlineEl = document.getElementById('axis2Inline');
            ...
            function filterOpp(axis1, axis2, opp) {
                return opp.filter(v => v !== axis1 && v !== axis2);
            }

            // Builder側に合わせる：axis1 + axis2 + opponents から unordered3（昇順固定）
            function buildKeys(axis1, axis2, opp) {
                const keys = [];
                if (!axis1 || !axis2) return keys;
                if (axis1 === axis2) return keys;
                if (opp.length < 1) return keys;

                for (const c of opp) {
                    const arr = [axis1, axis2, c].slice().sort((x, y) => (parseInt(x, 10) || 0) - (parseInt(y, 10) ||
                        0));
                    keys.push(`${arr[0]}-${arr[1]}-${arr[2]}`);
                }

                // 軸が入れ替わっても同じkeyになるので、念のためユニーク化
                return Array.from(new Set(keys)).sort((x, y) => {
            ...
            function updateAll() {
                const axis1 = selectedAxis1();
                const axis2 = selectedAxis2();

                const oppRaw = normalize(selectedOpponents());
                const opp = filterOpp(axis1, axis2, oppRaw);

                syncDisable(axis1, axis2);

                axis1InlineEl.textContent = axis1 ? axis1 : '未選択';
                axis2InlineEl.textContent = axis2 ? axis2 : '未選択';
            ...
            horseGrid.addEventListener('click', (e) => {
                ...
                if (col === 'axis1') {
                    const r = td.querySelector(`${axis1Selector}[value="${no}"]`);
                ...
                if (col === 'axis2') {
                    const r = td.querySelector(`${axis2Selector}[value="${no}"]`);
                    if (!r || r.disabled) return;
                    r.checked = true;
                    updateAll();
                    return;
            ...
            horseGrid.addEventListener('change', (e) => {
                if (
                    e.target.matches(axis1Selector) ||
                    e.target.matches(axis2Selector) ||
                    e.target.matches(oppSelector)
                ) {
                    updateAll();
                }
            });
