<x-app-layout title="購入馬券作成（{{ $race->name }}）">
    ...
    <style>
        #sanrenpukuNagashi1Axis .combo-scroll {
            scrollbar-width: thin;
        }

        #sanrenpukuNagashi1Axis .combo-scroll::-webkit-scrollbar {
            width: 6px;
            height: 6px;
        }

        #sanrenpukuNagashi1Axis .combo-scroll::-webkit-scrollbar-thumb {
            background: #cbd5e1;
            border-radius: 9999px;
        }

        #sanrenpukuNagashi1Axis .combo-scroll::-webkit-scrollbar-track {
            background: transparent;
        }
    </style>

    <div id="sanrenpukuNagashi1Axis" class="space-y-4">
        ...
        <form method="POST" action="{{ route('bet.cart.add', $race) }}" class="space-y-4">
            ...
            <div class="bg-white rounded-xl shadow-sm ring-1 ring-gray-200 p-4 space-y-4">
                <div class="flex items-center justify-between gap-3">
                    <h2 class="text-base font-semibold">三連複 1頭軸流し</h2>
                </div>
                    ...
                    <div class="lg:col-span-2">
                        <div class="flex items-end justify-between mb-2 gap-2">
                            <div>
                                ...
                                <div class="text-xs text-gray-500">軸は1頭、相手は2頭以上（軸と同じ馬は相手にできません）</div>
                            </div>
                        ...
                        <div class="sticky top-4 space-y-4">
                            ...
                            <div class="rounded-xl ring-1 ring-gray-200 bg-white overflow-hidden">
                                <div class="bg-indigo-600 text-white px-4 py-3">
                                    <div class="text-sm font-semibold">三連複 1頭軸流し</div>
                                </div>
    ...
    <script>
        (() => {
            ...
            // Builder側に合わせる：axis + opponents から unordered3 を作る
            function buildKeys(axis, opp) {
                const keys = [];
                const m = opp.length;
                if (!axis || m < 2) return keys;

                for (let i = 0; i < m; i++) {
                    for (let j = i + 1; j < m; j++) {
                        const arr = [axis, opp[i], opp[j]].slice().sort((x, y) => (parseInt(x, 10) || 0) - (parseInt(y,
                            10) || 0));
            ...
            function updateAll() {
                ...
                const opp = opponentsMinusAxis(axis, oppRaw);
                ...
                // ✅ ここが修正点：軸がないなら0点
                const t = axis ? comb2(opp.length) : 0;
