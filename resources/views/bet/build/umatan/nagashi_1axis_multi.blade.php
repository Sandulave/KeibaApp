<x-app-layout title="購入馬券作成（{{ $race->name }}）">
    ...
    <style>
        #umatanNagashi1AxisMulti .combo-scroll {
            scrollbar-width: thin;
        }

        #umatanNagashi1AxisMulti .combo-scroll::-webkit-scrollbar {
            width: 6px;
            height: 6px;
        }

        #umatanNagashi1AxisMulti .combo-scroll::-webkit-scrollbar-thumb {
            background: #cbd5e1;
            border-radius: 9999px;
        }

        #umatanNagashi1AxisMulti .combo-scroll::-webkit-scrollbar-track {
            background: transparent;
        }
    </style>

    <div id="umatanNagashi1AxisMulti" class="space-y-4">
        ...
        <form method="POST" action="{{ route('bet.cart.add', $race) }}" class="space-y-4">
            ...
            <input type="hidden" name="mode" value="{{ $mode ?? 'nagashi_1axis_multi' }}">

            <div class="bg-white rounded-xl shadow-sm ring-1 ring-gray-200 p-4 space-y-4">
                <div class="flex items-center justify-between gap-3">
                    <h2 class="text-base font-semibold">馬単 1頭軸流し（マルチ）</h2>
                </div>

                <div class="text-sm text-gray-600 leading-relaxed">
                    軸馬を1頭、相手を1頭以上選びます。<br>
                    マルチなので、相手1頭につき <strong>2通り（軸→相手 / 相手→軸）</strong> を展開してカートに入れます。
                </div>
                    ...
                    <div class="lg:col-span-2">
                        <div class="flex items-end justify-between mb-2 gap-2">
                            <div>
                                ...
                                <div class="text-xs text-gray-500">軸は1頭、相手は1頭以上（軸と同じ馬は選べません）</div>
                            </div>
                        ...
                        <div class="sticky top-4 space-y-4">
                            ...
                            <div class="rounded-xl ring-1 ring-gray-200 bg-white overflow-hidden">
                                <div class="bg-indigo-600 text-white px-4 py-3">
                                    <div class="text-sm font-semibold">馬単 1頭軸流し（マルチ）</div>
                                </div>
    ...
    <script>
        (() => {
            ...
            function updateAll() {
                ...
                selectedOppInlineEl.textContent = opp.length ? opp.join(' / ') : '未選択';
