<x-app-layout :title="$displayName . ' / ' . $race->name . ' の馬券詳細'">
    <div class="max-w-6xl mx-auto px-3 sm:px-6 lg:px-8 py-6 space-y-4">
        <div>
            ...
            <h1 class="mt-2 text-xl sm:text-2xl font-bold tracking-tight">{{ $displayName }} / {{ $race->name }} の馬券詳細</h1>
            ...
            <table class="min-w-[760px] w-full table-auto">
                <thead>
                    <tr class="bg-gray-50 border-b border-gray-200">
                        <th class="px-3 sm:px-4 py-3 text-left text-xs font-medium text-gray-600 uppercase tracking-wider">購入日時</th>
                        <th class="px-3 sm:px-4 py-3 text-left text-xs font-medium text-gray-600 uppercase tracking-wider">券種</th>
                        <th class="px-3 sm:px-4 py-3 text-left text-xs font-medium text-gray-600 uppercase tracking-wider">買い目</th>
                        <th class="px-3 sm:px-4 py-3 text-right text-xs font-medium text-gray-600 uppercase tracking-wider">金額</th>
                        <th class="px-3 sm:px-4 py-3 text-right text-xs font-medium text-gray-600 uppercase tracking-wider">払戻</th>
                        <th class="px-3 sm:px-4 py-3 text-center text-xs font-medium text-gray-600 uppercase tracking-wider">的中</th>
                    </tr>
                </thead>
                    ...
                    @forelse ($bets as $bet)
                        @foreach ($bet->items as $item)
                            <tr class="hover:bg-gray-50">
                                <td class="px-3 sm:px-4 py-3 whitespace-nowrap text-sm text-gray-700">{{ optional($bet->bought_at)->format('Y-m-d H:i:s') ?? '-' }}</td>
                                <td class="px-3 sm:px-4 py-3 whitespace-nowrap text-sm text-gray-900">{{ $betTypeLabels[$item->bet_type] ?? $item->bet_type }}</td>
                                <td class="px-3 sm:px-4 py-3 whitespace-nowrap text-sm font-mono text-gray-900">{{ $item->selection_key }}</td>
                                <td class="px-3 sm:px-4 py-3 whitespace-nowrap text-sm text-right text-gray-700">{{ number_format((int) $item->amount) }}円</td>
                                <td class="px-3 sm:px-4 py-3 whitespace-nowrap text-sm text-right text-gray-700">{{ number_format((int) ($item->return_amount ?? 0)) }}円</td>
                                <td class="px-3 sm:px-4 py-3 whitespace-nowrap text-sm text-center">
                                    @if ($item->is_hit)
